...
// scripts/audit-chart.ts
import SwissEph2 from "swisseph-wasm";

// lib/ephemeris.server.ts
...
var mockNatalChart = {
  event: {
    name: "Demo",
    title: "Carta natal",
    dateLabel: "23 abril 2026 \xB7 18:36",
    locationLabel: "Barcelona, Catalu\xF1a, Espa\xF1a",
    latitude: "41\xB0 23\u2032 19\u2033 N",
    longitude: "2\xB0 9\u2032 32\u2033 E",
    utcOffset: "UTC+02:00",
    timezone: "CEST",
    daylightSaving: "S\xED",
...
function ephemerisFlag() {
  return SEFLG_SPEED | SEFLG_SWIEPH;
}
function calcUt(se, jd, body, flags) {
  const arr = se.calc_ut(jd, body, flags);
  return {
    longitude: arr[0],
    latitude: arr[1],
    distance: arr[2],
...
function obliquity(julianDayUt) {
  const t = (julianDayUt - 2451545) / 36525;
  return 23.439291111 - 0.013004167 * t - 1638e-10 * t * t + 5036e-10 * t * t * t;
}
...
function clampHouse(housePosition) {
...
function getHouseForLongitude(se, longitude, latitude, houses, obliquityOfDate, eclipticLatitude = 0) {
...
function buildPoint(se, config, result, houses, latitude, obliquityOfDate) {
...
function buildDerivedPoint(se, id, glyph, longitude, color, houses, latitude, obliquityOfDate, retrograde = false) {
  const normalized = normalizeLongitude(longitude);
  return {
    id,
    glyph,
...
function detectAspects(points) {
  const drawablePointIds = /* @__PURE__ */ new Set([
    "sun",
    "moon",
    "mercury",
    "venus",
...
function isDiurnalSunHouse(house) {
...
async function calculateNatalChart(input) {
  ...
  if (!localDateTime.isValid) {
    throw new Error("Fecha, hora o zona horaria no v\xE1lidas");
  }
  ...
  const rawHouses = se.houses(julianDayUt, input.lat, input.lng, "P");
  const houseCusps = Array.from(rawHouses.cusps.slice(1, 13)).map((longitude, index) => ({
    house: index + 1,
    longitude: normalizeLongitude(longitude)
  }));
  const eps = obliquity(julianDayUt);
  const points = PLANET_CONFIG.map(
    (config) => buildPoint(se, config, calcUt(se, julianDayUt, config.body, flags), rawHouses, input.lat, eps)
  );
  const northNode = points.find((p) => p.id === "northNode");
  const sun = points.find((p) => p.id === "sun");
  const moon = points.find((p) => p.id === "moon");
  if (northNode) {
    points.push(buildDerivedPoint(
      se,
      "southNode",
      "\u260B",
  ...
  if (sun && moon) {
    ...
    const partOfFortuneLongitude = isDayChart ? rawHouses.ascmc[0] + moon.longitude - sun.longitude : rawHouses.ascmc[0] + sun.longitude - moon.longitude;
    points.push(buildDerivedPoint(
      se,
      "partOfFortune",
      "\u2297",
  ...
  return {
    event: {
      ...
      daylightSaving: input.daylightSaving ? "S\xED" : "No",
...
// scripts/audit-chart.ts
var SE_MEAN_NODE = 10;
var SE_TRUE_NODE2 = 11;
var SEFLG_SPEED2 = 256;
var SEFLG_SWIEPH2 = 2;
var SIGN_ABBR = {
  aries: "Ari",
  taurus: "Tau",
...
var SUBJECTS = [
  ...
  },
  {
    ...
    displayLocation: "Mil\xE1n, Italy",
    lat: 45.4642,
    lng: 9.19,
...
function formatPosition(longitude, retrograde) {
  const normalized = normalizeLongitude(longitude);
  ...
  return `${degree}\xB0 ${sign} ${minutes}'${retrograde ? " Rx" : ""}`;
}
function row(label, value) {
  return `${label.padEnd(10, " ")} | ${value}`;
}
async function getNodeLongitudes(julianDay) {
  const se = new SwissEph2();
  await se.initSwissEph();
  const flags = SEFLG_SPEED2 | SEFLG_SWIEPH2;
  const meanNode = se.calc_ut(julianDay, SE_MEAN_NODE, flags);
  const trueNode = se.calc_ut(julianDay, SE_TRUE_NODE2, flags);
  return {
    mean: normalizeLongitude(meanNode[0]),
    true: normalizeLongitude(trueNode[0])
  };
}
function pointMap(chart) {
  return new Map(chart.points.map((point2) => [point2.id, point2]));
}
async function main() {
  const lines = ["===== CHART AUDIT =====", ""];
  for (const [index, subject] of SUBJECTS.entries()) {
    ...
    lines.push(`Resolved UTC offset: ${formatOffset2(localTime.offset)}`);
    ...
    for (const body of BODY_ROWS) {
      const point2 = points.get(body.id);
      if (point2) {
        lines.push(row(body.label, formatPosition(point2.longitude, point2.retrograde)));
      }
    }
    ...
    for (const body of BODY_ROWS) {
      const point2 = points.get(body.id);
      lines.push(`  ${body.label}: ${String(point2?.retrograde)}`);
    }
    lines.push("");
  }
