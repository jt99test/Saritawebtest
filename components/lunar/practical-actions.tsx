      ...
      {actions ? (
        <div className="mt-9">
          {ACTION_CARDS.map((card, index) => (
            ...
            >
              <p className="text-[11px] uppercase tracking-[0.16em] text-dusty-gold/82">
                {card.title}
              </p>
              <p className="mt-3 pr-10 font-serif text-[21px] font-normal leading-[1.5] text-white">
                {actions[card.id]}
              </p>
              <p
                className={[
                  "absolute right-0 text-[10px] text-white/20 [font-variant-numeric:tabular-nums]",
                  index === 0 ? "top-0" : "top-6",
                ].join(" ")}
              >
        ...
        <p className="mx-auto mt-8 max-w-[440px] text-center font-serif text-[17px] italic leading-[1.7] text-white/50">
