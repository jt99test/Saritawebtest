  ...
  return (
    <section className="mx-auto mb-12 max-w-[760px] border-y border-dusty-gold/12 py-7">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <article className="border border-black/12 bg-white p-5 shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
          ...
          <div className="mt-3 grid gap-3">
            {patterns.map((pattern) => (
              <article key={`${pattern.type}-${pattern.points.join("-")}`} className="border-l border-dusty-gold/22 pl-4">
                ...
                <p className="mt-1 text-sm leading-7 text-[#3a3048]">{pattern.description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <p className="mt-6 text-xs leading-6 text-[#3a3048]">
