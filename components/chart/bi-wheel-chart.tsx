...
const VARIANTS = {
  "solar-return": {
    primary: "rgba(245,205,92,0.88)",
    muted: "rgba(245,205,92,0.62)",
    ring: "rgba(245,205,92,0.22)",
    bg: "rgba(245,205,92,0.055)",
  },
  synastry: {
    primary: "rgba(145,168,255,0.88)",
    muted: "rgba(145,168,255,0.66)",
    ring: "rgba(145,168,255,0.22)",
    bg: "rgba(145,168,255,0.055)",
  },
} as const;
...
function aspectStroke(type: string) {
  if (type === "trine" || type === "sextile") return "rgba(145,168,255,0.5)";
  if (type === "square" || type === "opposition") return "rgba(255,119,132,0.44)";
  return "rgba(245,205,92,0.45)";
}
      ...
      <svg viewBox="0 0 860 860" className="relative h-auto w-full overflow-visible" role="img" aria-label={outerChart ? `${innerLabel} / ${outerLabel}` : innerLabel}>
        <defs>
          <radialGradient id="bw-field-glow" cx="50%" cy="50%" r="50%">
            ...
            <stop offset="58%" stopColor="rgba(245,205,92,0.022)" />
        ...
        <circle cx={CENTER} cy={CENTER} r="392" fill="none" stroke={colors.bg} strokeWidth="16" filter="url(#bw-soft-halo)" />
        {outerChart ? (
          <>
            <circle cx={CENTER} cy={CENTER} r={OUTER_SEP_R} fill="none" stroke={colors.ring} strokeWidth="1.4" strokeDasharray="4 8" />
            <circle cx={CENTER} cy={CENTER} r={OUTER_PLANET_R} fill="none" stroke={colors.bg} strokeWidth="16" filter="url(#bw-soft-halo)" />
          </>
        ) : null}

        <g transform="translate(430 430) scale(0.78) translate(-430 -430)">
          <circle cx={CENTER} cy={CENTER} r="386" fill="rgba(6,8,14,0.66)" stroke="rgba(255,255,255,0.14)" />
          <circle cx={CENTER} cy={CENTER} r="356" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="18" />
          ...
          {innerChart.houses.map((house) => {
            ...
            return (
              <g key={house.house}>
                <line x1={CENTER} y1={CENTER} x2={line.x} y2={line.y} stroke="rgba(255,255,255,0.24)" />
                <line x1={cuspTick.x} y1={cuspTick.y} x2={cuspLabel.x} y2={cuspLabel.y} stroke="rgba(245,205,92,0.45)" />
                <text x={cuspLabel.x} y={cuspLabel.y} textAnchor="middle" dominantBaseline="central" className="text-[8px] font-semibold" fill="rgba(245,205,92,0.78)">
          ...
          <circle cx={CENTER} cy={CENTER} r="5" fill="rgba(255,255,255,0.75)" filter="url(#bw-glow)" />
